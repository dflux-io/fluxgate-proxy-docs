...
export default function Quickstart() {
  return (
    <DocPage
      ...
      lede="Build fluxgate-proxy from source, run it against the minimal config, exercise the admin API, and read the audit trail. About 10 minutes from a clean checkout."
    >
...
go build -o bin/fgpctl ./cmd/fgpctl`} />
      ...
      <p>
        ...
        outside it falls back to safe defaults: deny-by-default policy gate, in-memory audit
        store, in-memory shared-cache SQLite control plane, 30 s read timeout, 1 MiB request
        body cap.
      </p>
...
INF admin listener listening addr=127.0.0.1:8091`} />
      ...
      <Callout type="note" title="Where is the admin listener?">
        ...
        <code>127.0.0.1:8091</code>. The admin listener defaults to loopback-only and accepts
        cleartext h2c when <code>admin.allow_insecure: true</code> — fine for local dev, but
        you'll want TLS and API keys before exposing the admin port anywhere else. See{' '}
      ...
      <CodeBlock lang="bash" code={`./bin/fgpctl -url http://127.0.0.1:8091 status`} />
  ...
  "uptime_seconds": 17
      ...
      <p>
        ...
        rule — refine it before production):
      </p>
      <CodeBlock lang="json" code={`{
  "name": "allow-smoke",
  "action": "allow",
  "match": {
    "path_prefix": "/nudm-uecm/v1/"
  }
}`} />

      <CodeBlock lang="bash" code={`./bin/fgpctl -url http://127.0.0.1:8091 policy-rules add @allow-smoke.json`} />

      <h2 id="read-the-audit-trail">Read the audit trail</h2>
      <p>
        Send a request through the SBI listener — even one that 404s because no producer is
        registered — and FGP records the decision:
      </p>
      <CodeBlock lang="bash" code={`curl -i http://127.0.0.1:8090/nudm-uecm/v1/imsi-001010000000001/registrations

# then:
./bin/fgpctl -url http://127.0.0.1:8091 audit --limit 5`} />

      <p>
        Each record carries the request ID, decision (<code>allowed</code>/<code>denied</code>),
        deny reason if any, status code, target NF type, and duration in microseconds.
      </p>
      ...
      <p>
        See <Link to="/reference/metrics">Reference → Metrics</Link> for the full list of
        exported series.
      </p>
      ...
      <ul>
        <li>
          ...
          <Link to="/concepts/request-pipeline">request pipeline</Link> to understand what FGP
          is doing on every request.
        </li>
        <li>
        ...
        <li>
          Read <Link to="/guides/deploying">Deploying FGP</Link> for systemd, container, and
          persistence options.
        </li>
        <li>
